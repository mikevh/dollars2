...
public class AuthService
{
    ...
    public AuthService(UserRepository userRepo, RefreshTokenRepository refreshTokenRepo, IConfiguration config)
    {
        _userRepo = userRepo;
    ...
    public async Task<ApiResponse<AuthResponse>> LoginAsync(string email)
    {
        var user = await _userRepo.GetByEmailAsync(email);

        if (user is null)
        {
            return ApiResponse<AuthResponse>.Fail("User not found.", "USER_NOT_FOUND");
        }

        return await GenerateTokensAsync(user);
    }

    public async Task<ApiResponse<AuthResponse>> RefreshAsync(string refreshToken)
    {
        var token = await _refreshTokenRepo.GetValidTokenAsync(refreshToken);

        if (token is null)
        {
            return ApiResponse<AuthResponse>.Fail("Invalid or expired refresh token.", "INVALID_REFRESH_TOKEN");
        }
    ...
    private async Task<ApiResponse<AuthResponse>> GenerateTokensAsync(User user)
    {
